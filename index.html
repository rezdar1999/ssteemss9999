...
def main_screen():
    ...
    # أزرار للوظائف المختلفة
    tk.Button(main_window, text="إضافة منتج", command=open_add_product).grid(row=0, column=0)
    tk.Button(main_window, text="الكاشير", command=open_cashier).grid(row=0, column=1)
    tk.Button(main_window, text="المصروفات والإيرادات", command=open_financials).grid(row=1, column=0)
    tk.Button(main_window, text="الأرباح والخسائر", command=open_profit_loss).grid(row=1, column=1)
    tk.Button(main_window, text="إدارة المستخدمين", command=open_users).grid(row=2, column=0)
    tk.Button(main_window, text="تسجيل الخروج", command=logout).grid(row=2, column=1)
